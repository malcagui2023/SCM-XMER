...
function updateDashboardData() {
    fetch('/api/dashboard-data')
        ...
        .then(data => {
            ...
            // Display collection amount in US dollars instead of Brazilian reals
            if (todayAmountElement) {
                const amountThousands = (stats.collection_amount / 1000).toFixed(1);
                todayAmountElement.textContent = `$ ${amountThousands}K`;
            }
        })
        .catch(error => {
            ...
            resultElement.innerHTML = `
                <div class="calculation-results">
                    ...
                    <div class="result-item">
                        <strong>${labels.original}</strong> $ ${calc.original_amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </div>
                    <div class="result-item">
                        <strong>${labels.discount}</strong> $ ${calc.discount_amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </div>
                    <div class="result-item">
                        <strong>${labels.discounted}</strong> $ ${calc.discounted_amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </div>
                    <div class="result-item">
                        <strong>${labels.installments}</strong> ${calc.installments}x of $ ${calc.installment_value.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </div>
                    <div class="result-item total">
                        <strong>${labels.total}</strong> $ ${calc.total_amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </div>
                </div>
            `;
...
function performQuickSearch() {
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            // Always display debt amounts in US dollars for consistency
            const successMsg = currentLanguage === 'pt' ?
                `Cliente encontrado: ${customer.name}\nTelefone: ${customer.phone}\nDívida: $ ${customer.debt_amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}` :
                `Customer found: ${customer.name}\nPhone: ${customer.phone}\nDebt: $ ${customer.debt_amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}`;
